...
func NewParser(output chan<- base.SipMessage, errs chan<- error, streamed bool) Parser {
	...
	if !streamed {
		...
		log.Debug("Parser %p has bodylength chan %p", &p, &p.bodyLengths)
		p.bodyLengths.Init()
	}
...
func (p *parser) Write(data []byte) (n int, err error) {
	...
	if !p.streamed {
		log.Debug("%p Not streamed, getting body length", p)
		l := getBodyLength(data)
		p.bodyLengths.In <- l
		log.Debug("%p Body length is %d", p, l)
	}
...
func (p *parser) parse(lines <-chan string, requireContentLength bool) {
	...
	for {
		...
		if p.streamed {
			...
			log.Debug("Wait for content length in %p", p)
			contentLength = (<-p.bodyLengths.Out).(int)
			log.Debug("Got content length")
		}
		...
		for buffer.Len() < contentLength {
			...
			buffer.WriteString(line)
			buffer.WriteString("\r\n")
			log.Debug("%d bytes to go in parser %p", contentLength-buffer.Len(), p)
		}
...
func pipeLines(textIn <-chan string, linesOut chan<- string) {
	...
	processInput := func(input string) {
		...
		if len(lines) > 1 {
			// There was at least one CRLF in the input, so we can flush the buffer
			// and queue its contents to send, as we know that line is complete.
			toSend = append(toSend, buffer.String())
			buffer.Reset()
		}

		// We're dealt with line 0, so now queue lines 1 to n-1 (inclusive) to send.
		...
		if len(lines) > 1 {
			// Finally, write the contents of the last line into the buffer,
			// as we won't know when it's done until we receive the next CRLF.
